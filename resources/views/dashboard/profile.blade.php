    ...
    <form method="post" action="{{route('admin_profile')}}" id="admin_profile_form" class="card-panel hoverable" autocomplete="{{ Str::random(16) }}">
        ...
        <div class="row">
            <div class="input-field col s12 m3 l5 font-weight-bolder">
                Önkéntesség felfüggesztése
                <span class="helper-text">Tartós távollét, betegség(pozitív vírus teszt, karantén, stb), egyéb akadályoztatás esetén.</span>
            </div>
            ...
            <div class="input-field col s12 m4 center-align">
                <button class="waves-effect waves-light btn-large red" type="submit" name="action">Mentés
        ...
        @else
            <div class="row">
                <div class="input-field col s12 m4 center-align">
                    <a class="waves-effect red btn-large" href="?new_volunteer=1">Jelentkezem Vigyázónak
        ...
        @if(isset($current_user->volunteer_data) || Request()->new_volunteer == 1)
            <div class="row">
                @if(Request()->new_volunteer == 1)
                    <h4>Jelentkezés Vigyázónak</h4>
                @else
                    <h4>Vigyázói adataim</h4>
                @endif
            </div>
            <div class="row">
            ...
            <div class="row">
                ...
                <div class="input-field col s12 m4">
                    <select required class="validate" id="availability" name="availability">
                        <option value="" disabled @if(!isset($current_user->volunteer_data) || $current_user->volunteer_data->availability == null) selected @endif>Kérem válasszon</option>
                        <option @if(isset($current_user->volunteer_data) && $current_user->volunteer_data->availability == 'Délelőtt') selected @endif value="Délelőtt">Délelőtt</option>
                        <option @if(isset($current_user->volunteer_data) && $current_user->volunteer_data->availability == 'Délután') selected @endif value="Délután">Délután</option>
                        <option @if(isset($current_user->volunteer_data) && $current_user->volunteer_data->availability == 'Reggeltől-Estig') selected @endif value="Reggeltől-Estig">Reggeltől-Estig</option>
                        <option @if(isset($current_user->volunteer_data) && $current_user->volunteer_data->availability == '0-24 órában') selected @endif value="0-24 órában">0-24 órában</option>
                    </select>
                </div>
            </div>
            <div class="row">
                    ...
                    @for($i=1; $i<=min(count($current_user->volunteer_addresses), 4); $i++)
                        <div class="area-input-group row">
                            <div class="input-field col s5 m1">
                                ...
                                <label for="area_postcode_{{($i+1)}}">Irányítószám</label>
                            </div>
                            <div class="input-field col s7 m2">
                                ...
                                <label for="area_city_{{($i+1)}}">Város</label>
                            </div>
                            <div class="input-field col s12 m4">
                                ...
                                <label for="area_street_{{($i+1)}}">Utca</label>
                            </div>
                            <div class="input-field col s6 m2">
                                ...
                                <label for="area_from-house-number_{{($i+1)}}">Házszám(-tól)</label>
                                <span class="helper-text">Hagyja üresen, ha az egész utcát vállalja.</span>
                            </div>
                            <div class="input-field col s6 m2">
                                ...
                                <label for="area_to-house-number_{{($i+1)}}">Házszám(-ig)</label>
                                <span class="helper-text">Hagyja üresen, ha az egész utcát vállalja.</span>
                            </div>
